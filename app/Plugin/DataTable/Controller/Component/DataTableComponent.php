...
class DataTableComponent extends Component {
	...
	protected function _search(DataTableConfig $config, Model $Model) {
		...
		$searchTerm = Hash::get($params, 'sSearch');
                ...
                if(isset($Model->actsAs['Rot']['fields'])){
                    foreach ($Model->actsAs['Rot']['fields'] as $field){
                        $fieldsTochange[]=$Model->name.'.'.$field;
                    }
		...
		foreach ($config->columns as $column => $options) {
			if ($options['useField']) {
				...
				if ($columnSearchTerm && ($searchable === true || $searchable === DataTableConfig::SEARCH_COLUMN)) {
                                    ...
                                    //encrypt search Term start
                                    if(in_array($column, $fieldsTochange) && $columnSearchTerm!='' && isset($Model->Behaviors->Rot)){
                                        //var_dump($columnSearchTerm,$column); die;
                                        $tempSearch=$columnSearchTerm;
                                        $upperCase=  ucwords($tempSearch);
                                        $upperFirst= ucfirst($tempSearch);
                                        $lowerCase=strtolower($tempSearch);
                                        $columnSearchTerm=$Model->Behaviors->Rot->mapEncript($columnSearchTerm,$Model->Behaviors->Rot->_defaults['map_array']); 
                                        $columnSearchTerm2=$Model->Behaviors->Rot->mapEncript($upperCase,$Model->Behaviors->Rot->_defaults['map_array']);                                         
                                        $columnSearchTerm3=$Model->Behaviors->Rot->mapEncript($lowerCase,$Model->Behaviors->Rot->_defaults['map_array']);   
                                        $columnSearchTerm4=$Model->Behaviors->Rot->mapEncript($upperFirst,$Model->Behaviors->Rot->_defaults['map_array']);
                                        $conditions[]=array("((".$column." LIKE '%".$columnSearchTerm."%') OR (".$column." LIKE '%".$columnSearchTerm2."%') OR (".$column." LIKE '%".$columnSearchTerm3."%') OR (".$column." LIKE '%".$columnSearchTerm4."%'))");
        
                                    }else{
                                        $conditions[] = array("$column LIKE" => '%' . $columnSearchTerm . '%');
                                    }
                                    //encrypt search Term end
